...
一个非官方、非商业的雪之下雪乃风格 Codex v2 动态宠物项目。它采用正常动漫人体比例与 galgame 式克制表演：鼠标靠近时会双脚着地地避让、侧目并恢复端庄，而不是兴奋跳跃；事件动作通过相邻帧保持关键姿势，让责备、等待、专注与认可不会一闪而过。
...
- 悬停行为采用五帧克制避让：察觉、微退、连续两帧保持戒备侧目、平稳恢复；全程双脚着地。
- 应用端播放时序遵循固定 Codex v2 协议；本项目通过相邻帧重复关键姿势改善体感速度，而不是写入无效的自定义速度参数。
- 事件动作采用可读停顿：招呼 2 帧、悬停 2 帧、失败责备 4 帧、等待/工作/审阅各 3 帧保持核心姿势。
